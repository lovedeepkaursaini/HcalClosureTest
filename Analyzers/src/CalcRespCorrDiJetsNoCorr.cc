...
#include "HcalClosureTest/Analyzers/interface/CalcRespCorrDiJetsNoCorr.h"
...
CalcRespCorrDiJetsNoCorr::CalcRespCorrDiJetsNoCorr(const edm::ParameterSet& iConfig)
{
  // set parameters
...
CalcRespCorrDiJetsNoCorr::~CalcRespCorrDiJetsNoCorr()
{
}
...
void
CalcRespCorrDiJetsNoCorr::analyze(const edm::Event& iEvent, const edm::EventSetup& evSetup)
{ 
  ...
  for(reco::CaloJetCollection::const_iterator it=calojets->begin(); it!=calojets->end(); ++it) {
    ...
    jetcorretpairset.insert( JetCorretPair(jet, 1.0) );
...
void 
CalcRespCorrDiJetsNoCorr::beginJob(const edm::EventSetup&)
{
  // book histograms
...
void 
CalcRespCorrDiJetsNoCorr::endJob() {
...
double CalcRespCorrDiJetsNoCorr::deltaR(const reco::Jet* j1, const reco::Jet* j2)
{
...
DEFINE_FWK_MODULE(CalcRespCorrDiJetsNoCorr);
