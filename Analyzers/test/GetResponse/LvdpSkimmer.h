...
// Wed Dec 17 13:55:14 2014 by ROOT version 5.28/00c
// from TTree pf_gammajettree/tree for gamma+jet balancing using PFJets
// found on file: PhoJet_tree_CHS_check_1_1_if4.root
//////////////////////////////////////////////////////////

#ifndef LvdpSkimmer_h
#define LvdpSkimmer_h
...
public :
   ...
   virtual void     Loop();
   virtual void     Loop(bool doPUCorr, bool use2ndJet);
   virtual Bool_t   Notify();
   virtual void     Show(Long64_t entry = -1);
   virtual double calc_dphi(double phi1,double phi2);

};

#endif

#ifdef LvdpSkimmer_cxx
LvdpSkimmer::LvdpSkimmer(TTree *tree)
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("LvdpSele.root");
      if (!f) {
         f = new TFile("LvdpSele.root");
      }
      tree = (TTree*)gDirectory->Get("pf_gammajettree");

   }
   Init(tree);
}

LvdpSkimmer::~LvdpSkimmer()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t LvdpSkimmer::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t LvdpSkimmer::LoadTree(Long64_t entry)
{
...
void LvdpSkimmer::Init(TTree *tree)
{
   ...
   fChain->SetBranchAddress("pf_thirdjet_area", &pf_thirdjet_area, &b_pf_thirdjet_area);
...
void LvdpSkimmer::Show(Long64_t entry)
{
...
Int_t LvdpSkimmer::Cut(Long64_t entry)
{
